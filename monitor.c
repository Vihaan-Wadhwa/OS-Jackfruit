...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        strlcpy(entry->container_id, req.container_id, sizeof(entry->container_id));
    ...
    list_for_each_entry(entry, &monitored_list, list) {
        int pid_match = (req.pid > 0) ? (entry->pid == req.pid) : 0;
        int id_match = (req.container_id[0] != '\0') ?
                       (strncmp(entry->container_id, req.container_id,
                                sizeof(entry->container_id)) == 0)
                       : 0;

        if ((req.pid > 0 && req.container_id[0] != '\0' && pid_match && id_match) ||
            (req.pid > 0 && req.container_id[0] == '\0' && pid_match) ||
            (req.pid <= 0 && req.container_id[0] != '\0' && id_match)) {
            list_del(&entry->list);
            mutex_unlock(&monitored_lock);
            kfree(entry);
            return 0;
